namespace Repository.Pattern.Ef6.Factories
{
    ...
    public class RepositoryProvider : IRepositoryProvider
    {
        ...
        {
            return this.GetFactory(typeof(T))(); // get factory and invoke it in 1 fell swoop
        }

        public dynamic GetCustomRepository(Type type)
        {
            return this.GetFactory(type)(); // get factory and invoke it in 1 fell swoop
        }
        ...
        /// </remarks>
        public IRepositoryAsync<T> GetRepositoryForEntityType<T>()
            where T : class, IObjectState
        {
            return this.GetRepository<IRepositoryAsync<T>>(this.repositoryFactories.GetRepositoryFactoryForEntityType<T>());
        }
        ...
        public virtual T GetRepository<T>(Func<IDataContextAsync, IUnitOfWorkAsync, dynamic> factory = null) where T : class
        {
            ...
            if (this.Repositories.TryGetValue(typeof(T), out cachedRepository))
            {
                return cachedRepository;
            }

            // Not found or null; make one, add to dictionary cache, and return it.
            return this.MakeRepository<T>(factory, this.DataContext, this.UnitOfWork);
        }
        ...
        private Func<dynamic> GetFactory(Type type)
        {
            ...
            if (this.repositoryFactories.CustomRepositoryFactories.TryGetValue(type, out repository))
            {
                return repository;
            }
